...
void PetriNet::runGsmnMonitors() {
    ...
    for (int i = 0; i < gsmnMonitors.size(); i++) {
        gsmnDriver->set_objective(gsmnMonitors[i]);
        //gsmnDriver->updateLP();
...
void Substance::metaboliteAction(Driver *gsmn) {
    if (flag1) {
        gsmn->set_objective(gsmn_metabolite);
        //gsmn->updateLP();
...
void Substance::producibilityStateStandalone(Driver *gsmn) {
    if (gsmn->isUpdateRequired()) {
        gsmn->set_objective(gsmn_metabolite);
        //gsmn->updateLP();
        flux = gsmn->calcObjValue();
...
void Substance::producibilityStateBuffClient(Driver *gsmn) {
//    if (gsmn->isUpdateRequired()) {
        gsmn->set_objective(gsmn_metabolite);
        //gsmn->updateLP();
        flux = gsmn->calcObjValueBuff();
        //TODO
        //for (int i = 0; i < flux_requests.size(); i++) flux_requests[i].flux(gsmn);
//    }
    state = activity.activity(flux);
...
void Substance::objectiveAction(Driver *gsmn) {
    ...
    gsmn->add_change(gsmn_reaction, LB_INF, UB_INF);
    gsmn->set_objective(gsmn_reaction);
    //gsmn->updateLP();
    flux = gsmn->calcObjValue();
    state = activity.activity(flux);
    lb = flux - OBJECTIVE_TOLERANCE;
    ub = flux;
    gsmn->add_change(gsmn_reaction, lb, ub);
...
void Substance::setEnzymeBounds(Driver *gsmn) {
    ...
    for (int i = 0; i < gsmn_reactions.size(); i++) {
        ...
        gsmn->add_change(gsmn_reactions[i], lb * rweights[i], ub * rweights[i]);
...
void PetriNet::readModel(string path) {
    ...
    for (int i = 0; i < Ni; i++) {
        ...
        interactions[i].slow = false;
        interactions[i].reset = false;
        interactions[i].instant = false;
...
void PetriNet::runWorkflowStandalone(string path) {
    int cursor;
    fstream infile;
    string word;
    vector<string> text;
    ...
    while (cursor < text.size()) {
        ...
        if (text[cursor] == "PROPENSITY_FUNCTION") {
            ...
            cursor++;
            int iid = (-1);
            for (int i = 0; i < Ni; i++) if (interactions[i].getName() == text[cursor]) {
                    iid = i;
                    break;
                }
            if (iid == (-1)) {
                cout << "Unknown transition " << text[cursor] << " in propensity function" << endl;
                exit(0);
            }
            cursor++;
        ...
        if (text[cursor] == "HYBRID_SIMULATION3") {
            if (has_fba) hybridSimulation3Standalone();
...
void PetriNet::runWorkflowClient(string path, string serverAddress, string serverPort) {
    int cursor;
    fstream infile;
    string word;
    vector<string> text;
    ...
    while (cursor < text.size()) {
        ...
        //if (text[cursor] == "HYBRID_SIMULATION2") hybridSimulation2();
        if (text[cursor] == "HYBRID_SIMULATION3") {
            if (has_fba) hybridSimulation3Client(serverAddress, serverPort);
            if (!has_fba) pnSimulation();
        }
        if (text[cursor] == "SIMULATION") {
            if (has_fba) hybridSimulation3Client(serverAddress, serverPort);
...
double PetriNet::runServer(vector<string> text, string input_received) {
    int cursor = 0;
    string word;    
    while (cursor < text.size()) {
        ...
        //if (text[cursor] == "HYBRID_SIMULATION2") hybridSimulation2();
        if (text[cursor] == "HYBRID_SIMULATION3") {
            if (has_fba) return runOptimization(input_received);
            if (!has_fba) pnSimulation();
        }
        if (text[cursor] == "SIMULATION") {
            if (has_fba) return runOptimization(input_received);
...
void PetriNet::printJson(bool fast) {
    ...
    for (int i = 0; i < Ni; i++) {
        ...
        if (interactions[i].slow) {
            transition[INTERACTION_TAG][TYPE_TAG] = SLOW_TRANSITION_TAG;
        } else if (interactions[i].instant) {
            transition[INTERACTION_TAG][TYPE_TAG] = INSTANT_TRANSITION_TAG;
        } else if (interactions[i].reset) {
            transition[INTERACTION_TAG][TYPE_TAG] = RESET_TRANSITION_TAG;
        } else if (interactions[i].fba_flux) {
            transition[INTERACTION_TAG][TYPE_TAG] = FLUX_TRANSITION_TAG;
        } else {
            transition[INTERACTION_TAG][TYPE_TAG] = FAST_TRANSITION_TAG;
        }
