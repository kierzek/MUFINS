...
public:
    ...
    void producibilityStateBuffClient(Driver *gsmn);
    void producibilityStateStandalone(Driver *gsmn);  
...
private:
    ...
    Driver *gsmnDriver = NULL; // Driver class of the GSMN model
...
public:
    ...
    void run();
    void runWorkflowStandalone(string path);
    void runWorkflowClient(string path, string serverAddress, string serverPort);
    double runServer(vector<string> text, string input_received);
    ...
    void hybridSimulation3Client(std::string serverAddress, std::string serverPort); // Hybrid simulation without the notion of BIOMASS and NUTRIENT
    void hybridSimulation3Standalone();
    ...
    double runOptimization(std::string);
    double hybridSimulation3helper(std::string request);
};
