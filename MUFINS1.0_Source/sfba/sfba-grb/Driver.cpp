...
//void Driver::set_expfile(char *arg){
//    //kind = get_arg(string(arg));
//    strvec fname; //w/solver parameters
//    string tt(arg);
//    split2(&tt, &fname, COMMA);
//
//    if(fname.size() > 0) {
//        if(!fname[0].empty()) expfile=fname[0];
//        if(fname.size() > 1) {
//            if(!fname[1].empty()) metfile=fname[1];
//        }
//    }else{
...
ostream &operator << (ostream &out, OBJSTAT objstat){
    ...
    if(Driver::kind==GURB) out << objval << COLON << SPACE << Driver::get_msg_grb(objstat.status);//w/
