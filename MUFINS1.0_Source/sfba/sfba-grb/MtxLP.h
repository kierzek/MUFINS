...
protected:
    ...
    bool is_zero(double val)const{
        return (kind == EXACT) ? (val == 0) : ((abs(val)) <= epsilon); //IS_ZERO(val);
    }
...
public:
    ...
    int addCol_gfa(string name, double lb, double ub, int kind);//w/gfa/
...
//    void addMixedConstraint(stomap* sto, double lb = LB, double ub = UB, bool tmp = false, string name = "");
    void setColRange(string name, double lb, double ub, bool tmp = false);
    virtual void fix(string name, double value, bool tmp = false, string constraintname = "", double tol = 0);
    void fix(stomap *sto, bool tmp = false);
    void fix_gfa(string name, double value, bool tmp = false, string constraintname = "", double tol = 0);//w/gfa/
    ...
    virtual void addStoich(Matrix* N, stomap* sto, double lb = LB, double ub = UB, string name = "");//w/
    //virtual void addStoich(Model* model, stomap* sto, double lb, double ub, string name);//w/
    virtual void addStoich_gfa(stomap* sto, double lb = LB, double ub = UB, string name = "", int kind=CONT); //w/gfa/
    virtual void addStoich_gfa2(stomap* sto, double lb, double ub, string name, int kind);//w/
    virtual void addStoich_gni(stomap* sto, double lb = LB, double ub = UB, string name = "", int kind=CONT, string biom="BIOMASS"); //w/gni/
    virtual void addMatrix(Matrix* N);
    virtual void addMatrix_sig(Matrix* N);//w/
    //virtual void addMatrix(Matrix* N, Model* model);//w/
    virtual void addMatrix(Matrix* N, string problem, string biom="BIOMASS"); //w/
    void setColKind(string name, int kind);//w/gni/
    void addRC(string name, RC rc);//w/gni/
    ...
    void getRanges_gfva2(stomap* sol, rangemap* rm, OBJSTAT &objrange, strvec cols, bool presolve);//w/
    void fixSolution(stomap* obj, bool max = true, bool tmp = false, bool withzeroes = false, bool presolve = PRESOLVE, double scale = 1);
    void fixSolution_gfa(stomap* obj, bool max = true, bool tmp = false, bool withzeroes = false, bool presolve = PRESOLVE, double scale = 1);//w/gfa/
    void fixSolution_gfa2(stomap* sol, stomap* obj, bool max = true, bool tmp = false, bool withzeroes = false, bool presolve = PRESOLVE, double scale = 1);//w/
    void fixSolution_gfa3(stomap* sol, stomap* obj, bool max = true, bool tmp = false, bool withzeroes = false, bool presolve = PRESOLVE, double scale = 1);//w/
    void getOptRanges(rangemap* rm, OBJSTAT &obrange, stomap* obj, bool max, strvec queries, bool presolve = PRESOLVE);
    void getOptRanges_gim3e(rangemap* rm, stomap* obj, double value, strvec q, bool presolve = PRESOLVE);//w/gim3e
    void getOptRanges_gfa(rangemap* rm, OBJSTAT &obrange, stomap* obj, strvec queries, bool presolve = PRESOLVE);//w/gfa/
    void getOptRanges_gfa2(stomap* sol, rangemap* rm, OBJSTAT &obrange, stomap* obj, strvec queries, bool presolve = PRESOLVE);//w/
    void getOptRanges_gfa3(stomap* sol, rangemap* rm, OBJSTAT &obrange, stomap* obj, strvec queries,  bool max, bool presolve = PRESOLVE);//w/
    //void getShifts(rangemap* rm, stomap* obj, double const1, double const2, strvec queries, bool presolve = PRESOLVE);
    void getShifts(stomap *rv, stomap* obj, double const1, double const2, strvec queries, bool presolve = PRESOLVE);
   ...
   //liveness
    ...
    //flux coupling
    strvec getFullyCoupled(string targ, strvec query, double vmax, bool presolve = PRESOLVE);

    ///////////////////////////////////////////////////////////////////////////////////////
    //IO
    int writeLP(string filename);
    int writeMPS(string filename);
    FLUXACT getRange_gfaa(stomap* targ, bool presolve, string reac, double bound, double eps);//w/gfa/
