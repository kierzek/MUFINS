...
void PetriNet::updateFastReactions2() {
    ...
    for(i=0;i<Ns;i++)
		if(substances[i].getType()<4) {
            if(substances[i].stateChange==0 || substances[i].getState()==0) { substances[i].stateChange=0; continue;}
...
void PetriNet::updateFastReactions3(double maxdt) {
    ...
    for(i=0;i<Ns;i++)
		if(substances[i].getType()<4) {
            ...
            dt1 = maxChange*substances[i].getState()/abs(substances[i].stateChange);
            substances[i].stateChange = 0;
            if(dt1<dt && dt1>MIN_DT) { dt = dt1; sc = sc1; maxs = i; }
        }
...
void PetriNet::hybridSimulation2() {
	...
	for(i=0;i<Nsample;i++) {
		...
		while(Time<TimeMax) {
			if(msg_flag) cout << endl << "ITERATION " << Time << endl;
			if(substances[targetIdx].getState()>=targetThreshold) {Ntarget++; break;}
...
void PetriNet::hybridSimulation3() {
	int i,Ntarget;
	string rname;
	ofstream progfile;
    ...
    for(i=0;i<Nsample;i++) {
        ...
        iteration_counter=0;
		while(Time<TimeMax) {
			if(target_flag) if(substances[targetIdx].getState()>=targetThreshold) {Ntarget++;break;}
